...
## Purpose
Tổng hợp kết quả chạy test thành 1 narrative ngắn (≤300 từ). TÁI DÙNG dữ liệu đã parse sẵn trong `.state/deliverable-verifier.md` (do `qa-verifier` tạo ra) — KHÔNG tự parse lại `test-results.json` thô, tránh trùng logic với `agents/qa-verifier/tools/parse-test-results.js`.
...
## Variables
{{verifier_deliverable}} — toàn bộ `.state/deliverable-verifier.md` (đã có bảng TC_ID/status/nhãn)
