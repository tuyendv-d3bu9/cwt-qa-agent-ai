...
- **Severity** (mức độ nghiêm trọng KỸ THUẬT của lỗi với hệ thống — không phụ thuộc lịch làm việc): `Critical` / `Major` / `Minor` (giữ đúng 3 mức đã thấy trong dữ liệu thật `project-docs/05_QA/bug_export_S23.csv`, không tự thêm mức khác).
...
## Source
`Severity`/`Priority` là schema mới, tự khởi tạo cho QA Reporter (chưa có sẵn trong repo trước đây) — 3 mức Severity lấy từ giá trị thật quan sát trong `bug_export_S23.csv`; 3 mức Priority tự đề xuất, xác nhận cùng người dùng khi cần.
