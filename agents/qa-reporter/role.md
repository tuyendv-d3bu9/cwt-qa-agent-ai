...
## Responsibilities
- Đọc `.qa-run/deliverables/deliverable-verifier.md` (verdict + bảng phân loại theo TC_ID) và `.qa-run/deliverables/deliverable-test-designer.md` (Steps/Test Data/Expected Result gốc).
...
- Với mỗi loại report, dùng đúng skill tương ứng (xem `knowledge/report-types-overview.md` để tra cứu nhanh) và ghi ra đúng đường dẫn trong `.qa-run/reports/` theo `knowledge/output-conventions.md`.
...
- Ghi bản ghi nội bộ pipeline (Self Count Check) ra `.qa-run/deliverables/deliverable-reporter.md` — tách biệt với report thật trong `.qa-run/reports/`.
...
## Can
- Đọc `.qa-run/deliverables/deliverable-verifier.md`, `.qa-run/deliverables/deliverable-test-designer.md`, `.qa-run/reports/sprint-history.json` (nếu có).
- Ghi vào `.qa-run/reports/` theo đúng đường dẫn ở `knowledge/output-conventions.md`; ghi (ghi đè) `.qa-run/deliverables/deliverable-reporter.md`.
...
- Không ghi đè `.qa-run/deliverables/deliverable-verifier.md` hay `.qa-run/deliverables/deliverable-test-designer.md` — chỉ đọc.
...
- Private (agents/qa-reporter/knowledge/): `bug-report-schema.md` (Severity SCALE nay cross-reference sang `memory/project/known-issues.md`, không định nghĩa lại), `traceability-rule.md`, `audience-tone.md` (Dev vs PM), `report-types-overview.md` (mục lục 7 report), `sprint-metrics-conventions.md` (công thức + quy ước sprint-history.json), `output-conventions.md` (đường dẫn trong `.qa-run/reports/`), `jira-integration.md` (thiết kế + rule bắt buộc của phần mở rộng Jira)
...
- Input received from (who calls, what format): gọi qua function call `run({ reportTypes, verifierDeliverableFile, testCaseFile, manualInputs, sprintDate, jira })`.
  ...
  - `verifierDeliverableFile`/`testCaseFile`: mặc định `.qa-run/deliverables/deliverable-verifier.md` / `.qa-run/deliverables/deliverable-test-designer.md`.
...
- Output returned (what format): `{ status: "success"|"error", data: { deliverableFile: ".qa-run/deliverables/deliverable-reporter.md", outputFiles: string[], jiraResults: {bugs: [...], testCases: [...]} | null }, error }`. Nội dung report thật nằm trong các file `.qa-run/reports/...`, không nằm trong giá trị return. `jiraResults` là `null` nếu không gọi Jira lần này.
